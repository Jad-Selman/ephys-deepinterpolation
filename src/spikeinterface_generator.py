...
class SpikeInterfaceGenerator(SequentialGenerator):
    ...
    def __len__(self):
        ...
        return int(np.floor(len(self.list_samples) / self.batch_size) + 1)
    ...
    def generate_batch_indexes(self, index):
        ...
        if index < len(self) - 1:  # self.__len__()
            ...
            if max(indexes) > len(self.list_samples):
                print(index, min(indexes), max(indexes), len(self.list_samples))
