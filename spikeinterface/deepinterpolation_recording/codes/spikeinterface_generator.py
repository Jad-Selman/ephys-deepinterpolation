...
# TODO: rename to SpikeInterfaceRecordingGenerator
class SpikeInterfaceGenerator(SequentialGenerator):
    ...
    def __init__(self, recording, pre_frame=30, post_frame=30, pre_post_omission=1, desired_shape=(192, 2),
        ...
        "Initialization"
        assert recording.get_num_segments() == 1, "Only supported for mon-segment recordings"
    ...
    def generate_batch_indexes(self, index):
        ...
        if index < len(self) - 1: #self.__len__()
            ...
            shuffle_indexes = self.list_samples[-self.last_batch_size:]
        else:
            raise Exception(f"Exceeding number of available batches: {len(self)}")
    ...
    def __getitem__(self, index):
        ...
        input_full = np.zeros(
            [len(shuffle_indexes), self.desired_shape[0], self.desired_shape[1],
...
class SpikeInterfaceRecordingSegmentGenerator(SequentialGenerator):
    ...
    def __init__(self, recording_segment, start_frame, end_frame, pre_frame=30, post_frame=30, pre_post_omission=1, desired_shape=(192, 2),
                 batch_size=100, steps_per_epoch=10):
        "Initialization"
    ...
    def __data_generation__(self, index_frame):
        ...
        full_traces = self.recording_segment.get_traces(start_frame=start_frame, end_frame=end_frame,
                                                        channel_indices=channel_indices).astype("float32")
